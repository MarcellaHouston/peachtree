...
#### QUESTIONS TO ASK ####
# Do we need some form of sigmoid dampener within Glicko?
# Timing logic? Do I need to get yesterday's day?
# No need to store task rating? I'm not planning on changing task ratings after their initialization


def daily_glicko_update():
    users = db.select("users")
    today_name = date.today().strftime("%A").lower() # IMPORTANT: ASK WHEN THIS JOB IS BEING DONE, IF AT MIDNIGHT, MUST DO YESTERDAY
    for user in users:
        user_id = user[0]
        user_rating = user[4]
        user_RD = user[5]
        ...
        for task in today_task_list:
            ...
            task_rating, task_RD = calculate_task_glicko(task_data["impetus"], task_data["difficulty_score"])
...
def calculate_task_glicko(impetus: int, difficulty: int) -> tuple:
    ...
    task_rating = float(max(0, base_rating + impetus_effect))
