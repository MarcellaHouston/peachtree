...
try:
    ...
    print(
        "TIP: Ensure you are running from the 'src' directory where the 'backend' folder lives."
    )
    raise
...
def test_live_transcription_and_llm_summary():
    ...
    if not os.path.exists(TEST_AUDIO_FILE):
        pytest.skip(
            f" Skipping test: Please place a real audio file at {TEST_AUDIO_FILE} to run the AWS integration test."
        )
